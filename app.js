...
async function renderWallet(){
 ...
 el('sendTop').onclick=async()=>{if(!session?.user){renderLogin('سجّل الدخول أولاً حتى تتمكن من شحن المحفظة.');return}const amount=Number(el('ta').value);if(!(amount>0)||!el('tp').value.trim())return alert('أكمل البيانات');const x=await sb.from('wallet_topups').insert({user_id:session.user.id,amount,method:el('tm').value,sender_phone:el('tp').value.trim(),status:'pending'}).select('id').single();if(x.error){console.error('topup insert:',x.error);return alert('تعذر تسجيل طلب الشحن: '+(x.error.message||'خطأ غير معروف'))}let notifyFailed=false;try{const n=await sb.functions.invoke('discord-notify',{body:{type:'topup',id:x.data.id}});if(n.error){notifyFailed=true;console.warn('Discord topup notification error:',n.error)}else console.log('Discord topup notification sent:',n.data)}catch(e){notifyFailed=true;console.warn('Discord topup notification failed:',e)}alert(notifyFailed?'تم تسجيل طلب الشحن، لكن تعذر إرسال إشعار الإدارة.':'تم تسجيل طلب الشحن بنجاح');await renderWallet()}
}
