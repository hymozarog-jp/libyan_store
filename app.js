...
function parseCredential(code){
  ...
  if(raw.startsWith('{')&&raw.endsWith('}')){try{const obj=JSON.parse(raw);const login=String(obj.login??obj.email??obj.username??'').trim();const password=String(obj.password??'').trim();if(login&&password)return {login,loginLabel:obj.login_type==='username'?'username':'email',password};}catch(e){}}
...
async function addAccount(){const productId=el('codeProduct').value;const login=el('accountLogin').value.trim();const password=el('accountPassword').value;const msg=el('codeMsg');if(!productId||!login||!password)return alert('اختر المنتج واكتب الإيميل/اليوزر وكلمة السر');const select=el('codeProduct');const productLabel=select?.options?.[select.selectedIndex]?.textContent||'';if(!isCredentialProduct({name:productLabel,category:''}))return alert('هذا المنتج لا يستخدم نظام الحسابات');msg.textContent='جارٍ إضافة الحساب...';const code=JSON.stringify({login,password,login_type:/@/.test(login)?'email':'username'});const r=await sb.rpc('admin_upsert_product_code',{p_product_id:productId,p_code:code});if(r.error){msg.textContent='';return alert(r.error.message)}el('accountLogin').value='';el('accountPassword').value='';msg.textContent='تمت إضافة الحساب للمخزون ✓';renderAdmin()}
